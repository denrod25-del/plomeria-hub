...
This is a standalone repo with the Next.js app at its root, so Vercel works with zero path tweaks.

1. **Create the GitHub repo** (one time):
   - Go to **[github.com/new](https://github.com/new)**, name it e.g. `plomeria-hub`, leave it empty (no README), click **Create repository**.
   - On your computer, in this project folder, run the commands GitHub shows under *"…or push an existing repository"*:
     ```bash
     git remote add origin https://github.com/YOUR-USERNAME/plomeria-hub.git
     git branch -M main
     git push -u origin main
     ```
     *(This folder is already a git repo with one commit, so you can push immediately.)*
2. Go to **[vercel.com](https://vercel.com)** → sign in with GitHub → **Add New… → Project**.
3. **Import** your new `plomeria-hub` repository.
4. Vercel auto-detects **Next.js**. Leave **Root Directory** as the default (the repo root) and keep the default build settings.
